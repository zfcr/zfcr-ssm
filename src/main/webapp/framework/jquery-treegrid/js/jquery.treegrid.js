...
(function($) {
    ...
    var methods = {
        ...
        getSelectedNodeId: function() {
        	return $(this).treegrid('getSetting', 'selectedNodeId');
        },
        /**
         ...
         */
        isLeaf: function() {
        	var data = $(this).treegrid('getSetting', 'data');
        	var curNodeId = $(this).treegrid('getNodeId');
        	var curNodeIsLeaf = false;
        	$.each(data,function(i,idata){
        		if(data[i].id && curNodeId === data[i].id){
        			if(data[i].isLeaf != undefined && data[i].isLeaf == true){
        				curNodeIsLeaf = true;
        			}
        			return false;
        		}
        	});
        	return curNodeIsLeaf;
//            return $(this).treegrid('getChildNodes').length === 0;
        ...
        expand: function() {
            if (!this.treegrid('isLeaf') && !this.treegrid("isExpanded")) {
            	...
            	if(childNodes.length <= 0){
            		var raTemplate = this.treegrid('getSetting', 'refreshAnimateTemplate');
            		$(raTemplate).prependTo(cell);
            		this.treegrid("ajaxCallData");
            		var settings = this.treegrid('getTreeContainer').data('settings');
            		this.treegrid('initNode', settings);
            		$(cell).find(".glyphicon-refresh-animate").remove();
            		
            		// 选中行变色
            		$(cell).parent().siblings().click(function(){
            			var settings = $(this).treegrid('getTreeContainer').data('settings');
        ...
        ajaxCallData: function(){
    		...
    		if(ajaxReturnData == null || ajaxReturnData == undefined || 
    				!ajaxReturnData.data || ajaxReturnData.data.length <= 0){
    			return;
    		}
    		var htmlTr = methods.jointHtmlTr(this,ajaxReturnData.data);
    		$(this).after(htmlTr);
    		var settings = $(this).treegrid('getTreeContainer').data('settings')
    		var curData = ajaxReturnData.data;
    		$.each(settings.data,function(i,idata){
    			curData.push(idata);
    		});
    		settings.data = curData;
    		this.treegrid('setSettings', settings);
        },
        jointHtmlTr: function(treegridObj,treeData){
			var htmlTextData = "";
			var idColName = treegridObj.treegrid('getSetting', 'idColName');
			var treeColName = treegridObj.treegrid('getSetting', 'treeColName');
			var columns = treegridObj.treegrid('getSetting', 'columns');
			var textAlign = treegridObj.treegrid('getSetting', 'textAlign');
			for(var i = 0; i < treeData.length; i++){
				...
				var settings = $(treegridObj).treegrid('getTreeContainer').data('settings');
				if (settings.operates && settings.operates.length > 0) {
					colTd += methos_zf.initOperates(settings,treeData[i]);
				}
    ...
    $.fn.treegrid.defaults = {
        ...
        refreshAnimateTemplate: '<span class="glyphicon glyphicon-refresh glyphicon-refresh-animate"></span>',
